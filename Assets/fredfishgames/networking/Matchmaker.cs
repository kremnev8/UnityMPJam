...
namespace fredfishgames.networking
{
    ...
    public class Matchmaker
    {
        ...
        public Thread SendServer(Action<string> callback, IPAddress address, ushort port)
        {
            List<Byte> command = new List<byte>();
            command.Add(4);
            command.AddRange(address.GetAddressBytes());
            command.AddRange( BitConverter.GetBytes(IPAddress.HostToNetworkOrder(port)));
            command.Add(0);
            Thread thread = new Thread(() => sendServer(threads.Count - 1,command.ToArray()));
        ...
        {
            Thread thread = new Thread(() => sendServer(threads.Count - 1, new Byte[8] {2, 0, 0, 0, 0, 0, 0, 0 }));
        ...
        {
            Thread thread = new Thread(() => sendServer(threads.Count - 1, new Byte[8] { 5, 0, 0, 0, 0, 0, 0, 0 }));
        ...
        private void getServer(int threadId) {
            ...
            try
            {
                ...
                byte[] adrs = sendCommand(Address, Port, new Byte[8] { 3, 0, 0, 0, 0, 0, 0, 0 });
                
                if (adrs[0] == 1)
                {
                    ushort port = (ushort)IPAddress.NetworkToHostOrder(BitConverter.ToInt16(adrs.AsSpan(5, 2)));
                    ip = $"{adrs[1]}.{adrs[2]}.{adrs[3]}.{adrs[4]}:{port}";
                }
                else
                {
                    ip = "0";
