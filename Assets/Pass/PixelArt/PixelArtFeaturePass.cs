...
{
    public class PixelArtFeaturePass : ScriptableRenderPass
    {
        public static bool doPixelate;
        ...
        FilteringSettings defFilteringSettings;

        static int pixelTexID = Shader.PropertyToID("_PixelTexture");
        static int pixelDepthID = Shader.PropertyToID("_DepthTex");

        static int cameraID = Shader.PropertyToID("_CameraColorAttachmentA");

        public PixelArtFeaturePass(RenderPassEvent renderEvent, Material bM, float pD, int lM, Material outlineMat) {
            m_ProfilingSampler = new ProfilingSampler("PixelArtFeature");
            ...
            defFilteringSettings = new FilteringSettings(RenderQueueRange.opaque);
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            CommandBuffer cmd = CommandBufferPool.Get("PixelArtFeature");
            using (new ProfilingScope(cmd, m_ProfilingSampler))
            {
                if (doPixelate)
                {
                   ...
                   // cmd.DrawMesh(mesh, Matrix4x4.identity, _outlineMat, 0, 0);
                ...
                {
                    context.DrawRenderers(renderingData.cullResults, ref drawingSettings, ref defFilteringSettings, ref m_RenderStateBlock);
                    var mesh = RenderingUtils.fullscreenMesh;
                    cmd.DrawMesh(mesh, Matrix4x4.identity, _outlineMat, 0, 0);
